...
class res_config_settings_wallet(models.TransientModel):
    _inherit = 'res.config.settings'
    
    is_website_wallet = fields.Boolean(string="Allow Website Wallet")


    @api.model
    def default_get(self, fields_list):
        res = super(res_config_settings_wallet, self).default_get(fields_list)
        if self.search([], limit=1, order="id desc").is_website_wallet:
            res.update({'is_website_wallet': self.search([], limit=1, order="id desc").is_website_wallet})
...
class pos_wallet_transaction(models.Model):
    ...
    def wallet_transaction_email_send(self):
        ...
        if self.wallet_type == 'credit':
            template_id = self.env['ir.model.data'].get_object_reference('bi_internal_wallet', 'email_template_wallet_transaction_credit')[1]
        else:
            template_id = self.env['ir.model.data'].get_object_reference('bi_internal_wallet', 'email_template_wallet_transaction_debit')[1]
...
class Website(models.Model):
    ...
    def wallet_res_config(self):
        website_config_obj = self.env['res.config.settings']
        website_wallet_config = website_config_obj.search([], limit=1, order="id desc").is_website_wallet
        return website_wallet_config       
